...
<Tabs groupId="framework">
  ...
  <TabItem value="react-native" label="React Native">
    ...
    ```bash
    npm install react-native-document-picker react-native-file-viewer react-native-fs react-native-safe-area-context react-native-sound-player react-native-svg react-native-webview
    ```

    Next, install the corresponding Cloudflare RealtimeKit packages:

    ```bash
    npm install @cloudflare/realtimekit @cloudflare/realtimekit-react-native-core @cloudflare/realtimekit-react-native-ui-kit @cloudflare/react-native-webrtc
    ```
